...
for brand, url in second_urls:
    ...
    items = soup.select('div.product-card__vertical')
    print(f"► 本次共找到 {len(items)} 張 2nd STREET「{brand}」product-card__vertical")

    for item in items:
        try:
            # 名稱
            name_tag = item.select_one('h3.product-card__vertical__name')
            name = name_tag.text.strip() if name_tag else ""

            # 連結
            link_tag = item.select_one('a.product-card__vertical__link')
            ...
            # 價格
            price_tag = item.select_one('p.product-card__vertical__price')
            price = price_tag.text.strip().replace('\n', '') if price_tag else ""

            # 圖片
            img_tag = item.select_one('img.product-card__vertical__media')
            raw_img = img_tag['src'] if img_tag and img_tag.has_attr('src') else ""
...
# ===== 判斷 新品/價格異動 only（從 Google Sheets 讀取 last_seen） =====
